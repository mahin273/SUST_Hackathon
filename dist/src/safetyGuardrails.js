 ...
 */
function applySafetyGuardrails(request, response) {
        ...
        for (const numStr of numbersInComplaint) {
            ...
            if (!isNaN(amount)) {
                ...
                if (matchingTxns.length >= 2) {
                    ...
                    const mentionsCounterparty = matchingTxns.some(txn => request.complaint.includes(txn.counterparty) ||
    ...
    if (refundConfirmFoundInReply) {
        if (isBangla) {
            customerReply = customerReply.replace(/(রিফান্ড\s+করা\s+হয়েছে|রিফান্ড\s+করে\s+দিব|রিফান্ড\s+দেওয়া\s+হলো|টাকা\s+ফেরত\s+দেওয়া\s+হয়েছে|আমরা\s+ফেরত\s+দিব)/g, "যেকোনো যোগ্য পরিমাণ অর্থ অফিসিয়াল চ্যানেলের মাধ্যমে ফেরত দেওয়া হবে");
        }
        else {
            customerReply = customerReply.replace(/\b(will\s+refund\s+you|refunded\s+your\s+money|reversal\s+is\s+done|refund\s+has\s+been\s+processed|we\s+will\s+refund)\b/gi, "any eligible amount will be returned through official channels");
        }
    ...
    if (refundConfirmFoundInAction) {
        recommendedNextAction = recommendedNextAction.replace(/\b(refund\s+the\s+customer|reverse\s+transaction|unblock\s+account)\b/gi, "initiate verification workflow per policy before routing for official reversal/unblock");
    }
